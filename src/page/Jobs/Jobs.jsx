        ...
        <div className="background1 relative min-h-screen">
            ...
            <main className="relative z-10 pb-24 md:pb-12 px-4 md:px-10 md:pl-[110px]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-3 items-center">
                        ...
                        <div className="relative w-full">
                            <input 
                                ...
                                className="w-full h-[48px] md:h-[56px] rounded-[8px] border-2 border-[#2A3B31] bg-[#9FE870]/5 px-4 text-sm md:text-base text-[#D0D3D1] placeholder-[#707972] font-space-grotesk outline-none transition focus:border-[#3A5243]" 
                            />
                        </div>
                        ...
                        <div className="relative w-full">
                            <div 
                                ...
                                className="flex items-center justify-between w-full h-[48px] md:h-[56px] px-4 bg-[#9FE870]/5 border-2 border-[#2A3B31] rounded-[8px] cursor-pointer transition-colors duration-200 hover:border-[#3A5243]"
                            >
                                <span className="text-[#D0D3D1] text-sm md:text-base font-space-grotesk">{selected}</span>
                                <img 
                                    src={down2} 
                                    alt="Dropdown Arrow" 
                                    className={`w-[22px] h-[22px] pointer-events-none transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} 
                                />
                            </div>
                            {isOpen && (
                                <ul className="absolute z-20 w-full mt-1 bg-[#1E2924] border-2 border-[#2A3B31] rounded-[8px] overflow-hidden shadow-lg">
                                    <li 
                                        onClick={() => { setSelected("Chicago, IL"); setIsOpen(false); }} 
                                        className="px-4 py-3 cursor-pointer text-[#D0D3D1] text-sm md:text-base font-space-grotesk hover:bg-[#2A3B31] transition"
                                    > 
                                        Chicago, IL 
                                    </li>
                                    <li 
                                        onClick={() => { setSelected("New York, NY"); setIsOpen(false); }} 
                                        className="px-4 py-3 cursor-pointer text-[#D0D3D1] text-sm md:text-base font-space-grotesk hover:bg-[#2A3B31] transition"
                                    > 
                                        New York, NY 
                                    </li>
                                    <li 
                                        onClick={() => { setSelected("Remote"); setIsOpen(false); }} 
                                        className="px-4 py-3 cursor-pointer text-[#D0D3D1] text-sm md:text-base font-space-grotesk hover:bg-[#2A3B31] transition"
                                    > 
                                        Remote 
                                    </li>
                ...
                <section className="pt-8 md:pt-16 flex flex-col items-center">
                    ...
                    <div className="pt-6 md:pt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-[1200px] w-full mx-auto px-1 md:px-4">
                        {(showAll ? jobs : jobs.slice(0, 3)).map((job) => (
                            <div 
                                ...
                                className="w-full p-[2px] rounded-xl cursor-pointer group hover:bg-gradient-to-r hover:from-[#9FE870] hover:to-[#15CF9D] transition-all duration-300"
                            >
                                    ...
                                    <div className="flex justify-between items-center pt-2 border-t border-white/5">
                                        <div className="flex flex-wrap gap-1.5">
                                            {job.tags.map((tag, index) => (
                                                <span 
                                                    key={index} 
                                                    className="px-2.5 py-1 text-[11px] font-bold font-space-grotesk rounded-[4px] bg-[#343633] border border-[#2F312E] text-[#D6D6D6]"
                                                >
                                                    {tag}
                                                </span>
                                            ))}
                    ...
                    {/* See All/Fewer Button */}
                    <button 
                        onClick={() => setShowAll(!showAll)} 
                        className="group relative mt-6 md:mt-8 rounded-full p-[2px] bg-gradient-to-r from-[#9FE870] to-[#15CF9D] transition-all duration-300 ease-out hover:-translate-y-[2px] active:opacity-75 outline-none w-[200px]"
                    >
                ...
                <section className="p-4 md:p-10 mt-12 min-h-[300px] max-w-[1200px] mx-auto">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 justify-items-center">
                        {topCompanies.map((company, index) => (
                            <div 
                                key={index} 
                                className="flex items-center gap-4 w-full max-w-[320px] bg-white/[0.02] md:bg-transparent p-4 md:p-0 rounded-xl border border-white/[0.05] md:border-none"
                            >
                                ...
                                <div>
                                    ...
                                    <button className="pt-2 text-[13px] md:text-[14px] text-[#9FE870] font-space-grotesk font-medium transition-all duration-300 hover:underline hover:text-[#B5F88F] hover:translate-x-1 block"> 
                                        View Jobs 
                                    </button>
                                </div>
                            </div>
