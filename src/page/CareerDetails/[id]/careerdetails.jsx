...
export default function CareerDetails({
  ...
  // Convert both IDs to strings to prevent type mismatch issues
  const career = roles.find(role => String(role.id) === String(id));

  if (!career) {
    return (
      <div className="min-h-screen bg-[#0E0F0E] flex items-center justify-center">
        <p className="text-white p-10 font-space-grotesk text-center">Career path not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-screen bg-[#0E0F0E] text-white relative px-4 sm:px-6 pb-28 md:pb-8 overflow-clip">
      ...
      <main className="lg:pl-[120px] lg:pr-[40px] mt-4 md:mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">

        <div className="lg:col-span-2 space-y-6 md:space-y-[24px] mt-[10px] min-w-0">

          <section className="relative">
            <img src={imagebl} alt="Top gradient border detail" className="absolute top-[-8px] left-0 w-full h-[16px] md:h-[22px] object-cover rounded-t-[32px]" />
            <div className="relative bg-[#1E211D] rounded-[16px] py-5 px-4 md:py-[32px] md:px-[20px] border border-[#2A2D29] z-10 space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start gap-3 sm:items-center">
                <h2 className="text-2xl sm:text-3xl md:text-[32px] font-space-grotesk font-bold tracking-[-0.025em] leading-tight"> {career.title} </h2>
                <div className="flex items-center gap-2 font-bold text-sm sm:text-base md:text-[20px] font-space-grotesk flex-shrink-0 bg-[#252924] sm:bg-transparent px-3 py-1.5 sm:p-0 rounded-full border border-[#313630] sm:border-transparent">
                  <span>✅</span>
                  <span className="bg-[linear-gradient(90deg,#9FE870_17%,#15CF9D_100%)] bg-clip-text text-transparent"> {career.vibe || "80%"} vibe check </span>
                </div>
              </div>
              <p className="max-w-[820px] font-space-grotesk text-sm md:text-[16px] tracking-[-0.015em] text-[#A4A6A4] leading-relaxed"> {career.description} </p>
              <div className="pt-2">
                <h3 className="max-w-[820px] font-space-grotesk text-sm sm:text-[15px] md:text-[18px] tracking-[-0.015em] text-[#D2D3D2] font-medium"> Required Skills </h3>
                <div className="flex flex-wrap gap-2 mt-3">
                  {career.skills?.map((skill, index) => (
                    <span key={index} className="px-3 py-1.5 md:px-[12px] rounded-full bg-[#343633] border border-[#3E403D] text-[#FFFFFF] font-space-grotesk text-xs md:text-sm shadow-sm transition-colors hover:bg-[#3e403d]" > {skill} </span>
                  ))}
                </div>
              </div>
            </div>
          </section>
          <section className="bg-[#1D201C] p-4 md:p-6 rounded-xl border border-[#232722] font-space-grotesk space-y-4">
                ...
                <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between text-white gap-3 p-3 sm:p-0 rounded-lg bg-[#171916] sm:bg-transparent border border-[#222521] sm:border-transparent">
                  <div className="w-full sm:w-1/4 flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="text-sm md:text-[16px] font-medium tracking-[0.01em] text-[#D2D3D2] truncate"> {item.level} </span>
                      <span className="text-xs md:text-[14px] tracking-[0.015em] text-[#A4A6A4] truncate"> {item.experience} </span>
                    </div>
                  </div>
                  <div className="w-full sm:w-2/3 flex items-center sm:mx-4">
                    <div className="relative w-full h-7 bg-[#1C2E26] rounded-md overflow-hidden border border-[#213a2f]">
                      <div className="absolute top-0 left-0 h-full bg-[#0AB280] rounded-r-sm transition-all duration-500" style={{ width: item.percentage }} />
                    </div>
                  </div>
                  <div className="w-full sm:w-auto flex items-center justify-between sm:justify-end text-right sm:flex-shrink-0 pt-2 sm:pt-0 border-t border-[#232722] sm:border-0">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-bold sm:font-medium text-lg md:text-[24px] text-[#9FE870]"> {item.salary} </span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <section className="p-4 md:p-[20px] bg-[#1E211D] text-white rounded-[16px] border border-[#2A2D29] shadow-2xl space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
              <div className="space-y-1">
                <h2 className="font-bold text-base md:text-[24px] font-space-grotesk tracking-tight text-[#A5A6A5]">Demand</h2>
              ...
              <div className="w-full sm:w-1/2 relative pt-2">
                ...
                <div className="mt-2 w-full flex justify-between text-[10px] sm:text-xs font-semibold font-space-grotesk text-[#787977] px-1">
            ...
            <div className="bg-[#181A17] p-4 md:p-[24px] border border-[#324228] rounded-xl space-y-4">
              <h3 className="text-sm sm:text-base md:text-[18px] font-semibold text-white font-space-grotesk"> Top Companies hiring product designers </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {companies.map((company, index) => (
                  <div key={index} className="flex items-center gap-3 p-2 rounded-lg bg-[#1E211D] border border-[#2A2D29] sm:bg-transparent sm:border-0">
                    ...
                    <div className="flex flex-col min-w-0">
                      <h4 className="text-sm sm:text-[16px] font-medium text-white tracking-tight truncate"> {company.name} </h4>
                      <span className="text-xs font-space-grotesk text-[#A2A3A2] truncate"> {company.openPositions} open positions </span>
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => navigate("/careerlisting")} className="group relative mt-2 rounded-full p-[2px] bg-gradient-to-r from-[#9FE870] to-[#15CF9D] transition-all duration-300 ease-out hover:-translate-y-[2px] active:opacity-75 outline-none w-full sm:w-auto">
          ...
          <section className="rounded-[16px] bg-[#1B1E1B] p-4 md:p-[20px] border border-[#232722] text-[#E6E6E6] space-y-4">
            <h2 className="text-lg md:text-[22px] font-bold font-space-grotesk tracking-tight leading-snug"> A typical day at work </h2>
            <div className="space-y-3">
              {workDayData.map((item, index) => (
                <div key={index} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 p-3 rounded-xl bg-[#141614] border border-[#222521] sm:bg-transparent sm:border-0 sm:p-0">
                  ...
                  <div className="text-sm sm:text-[14px] text-[#FFFFFF] font-space-grotesk leading-relaxed flex-1 pl-0.5 sm:pl-0"> {item.title} </div>
                </div>
              ))}
            </div>
          </section>
          <section className="w-full rounded-[16px] bg-[#1B1E1B] border border-[#232722] p-4 md:p-[20px] space-y-4">
            <h2 className="text-lg md:text-[24px] font-bold text-white font-space-grotesk tracking-tight"> Popular Product Designers in the US </h2>
              ...
              {popularDesigners.map((designer, index) => (
                <div key={index} className="flex items-center gap-3 rounded-xl border border-[#283122] bg-[#1B1D1A] p-3 w-full min-w-0 transition-transform duration-200 hover:scale-[1.01]">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-white font-space-grotesk tracking-tight truncate"> {designer.name} </p>
                    <p className="text-xs text-[#A4A6A4] font-space-grotesk truncate"> {designer.title} </p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="space-y-4">
            <h2 className="text-xl md:text-[32px] font-space-grotesk font-bold mt-4 tracking-tight"> Career paths Recommended by Flex </h2>
            <div className="flex flex-wrap gap-3 md:gap-2 mx-auto transition-all duration-300" >
              {roles.slice(0, 2).map((role, i) => (
                <article key={i} className="p-3 md:p-5 rounded-2xl shadow-xl w-full sm:max-w-[340px]" style={{ backgroundColor: role.cardBg }} >
                  <h1 className="font-space-grotesk font-bold text-black pb-3 md:pb-4 flex justify-center text-[21px] md:text-[30px] leading-snug text-center"> {role.title} </h1>
        ...
        <aside className="w-full bg-[#10130F] rounded-xl p-5 md:p-[24px] border border-[#587E40] space-y-5">
          <h2 className="font-space-grotesk font-bold text-2xl sm:text-3xl lg:text-[32px] leading-tight">
            <span className="bg-gradient-to-r from-[#9FE870] to-[#15CF9D] bg-clip-text text-transparent tracking-[-0.015em]"> Ready to earn $180k a year? </span>
            <span className="ml-1" role="img" aria-label="Rich Face">🤑</span>
          </h2>
          ...
          <button onClick={() => navigate("/learningpath")} className="group relative rounded-full w-full overflow-hidden p-[2px] bg-gradient-to-r from-[#9FE870] to-[#15CF9D] transition-all duration-300 ease-out hover:-translate-y-[2px] active:opacity-75 outline-none block" >
